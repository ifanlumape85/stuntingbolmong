            ...
            <div class="col-md-12">
                <div class="card card-primary">
                    <div class="card-header">
                        <h3 class="card-title">Change Puskesmas</h3>
                    </div>
                    <!-- /.card-header -->
                    <!-- form start -->
                    <form method="post" action="{{ route('puskesmas.update', $item->id) }}">
                        @method('PUT')
                        @csrf
                        @include('pages.puskesmas.partials.form-control', ['submit' => 'Update'])
                    </form>
                </div>
            </div>
        </div>
